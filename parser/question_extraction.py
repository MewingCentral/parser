from parser.model import Section, Question, Page, SectionType, questions_as_string, PageType, SubQuestion
...
def get_questions(section: Section) -> List[Question]:
    ...
    for i, page in enumerate(section.pages):
        ...
        next_plus_current = apply_header_filter(
...
def extract_sub_questions(text: str) -> List[SubQuestion]:
    ...
    for match in matches:
        ...
        assert sum(x is not None and x != "" for x in [letter, letter_alt, letter_alt2]) == 1, f"Only one of letter, letter_alt, and letter_alt2 should be not None. letter='{letter}', letter_alt='{letter_alt}', letter_alt2='{letter_alt2}'"
